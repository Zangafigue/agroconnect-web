<<<<<<< Updated upstream
export default function AdminDisputeResolvePage() { return <div>Admin Dispute Resolve</div>; }
=======
...
export default function AdminDisputeResolvePage() {
  ...
  const { disputes, fetchDisputes, resolveDispute } = useDisputeStore();
  ...
  }, [fetchDisputes, disputes.length]);

  useEffect(() => {
    const foundDispute = disputes.find(d => d.id === id || String(d.id) === id);
    if (foundDispute) {
      setDispute(foundDispute);
    } else if (id === 'LTG-882' || id === 'LTG-881' || id === 'L012') {
        const mockDisputes = [
            {
              id: "LTG-882",
              plaintiff: "Fatima T.",
              defendant: "Koné D.",
              reason: "Non livraison",
              status: "EN COURS D'EXAMEN",
              quoting: "J'attends ma livraison depuis 3 jours. Le transporteur ne répond plus à mes appels alors que le chargement de maïs de 2 tonnes a été validé au hangar.",
              plaintiffRole: "Acheteur",
              defendantRole: "Transporteur"
            }
        ];
        setDispute(mockDisputes.find(d => d.id === id) || mockDisputes[0]);
    }
  }, [disputes, id]);

  const handleResolve = async () => {
    if (!decision) return alert('Veuillez prendre une décision');
    try {
      await resolveDispute(id, { decision, note });
  ...
  if (!dispute && id !== 'LTG-882' && id !== 'LTG-881' && id !== 'L012') {
    return (
        <div className="flex flex-col items-center justify-center py-20">
            <div className="w-10 h-10 border-4 border-primary border-t-transparent rounded-full animate-spin mb-4"></div>
            <p className="text-on-surface-variant font-bold">Chargement du litige...</p>
        </div>
    );
  }
  ...
  const currentDispute = dispute || {
      id: id,
      plaintiff: "Utilisateur",
      defendant: "Utilisateur",
      reason: "Inconnu",
      status: "INCONNU"
  };
          ...
          <section className="bg-surface-container-lowest rounded-xl p-8 shadow-sm border border-outline-variant/10">
            ...
            <div className="grid grid-cols-2 gap-y-6 gap-x-12">
              <div>
                ...
                <p className="font-medium text-on-surface">{currentDispute.plaintiff}</p>
              </div>
              <div>
                <p className="text-[10px] uppercase tracking-widest text-outline mb-1 font-bold">Défendeur</p>
                <p className="font-medium text-on-surface">{currentDispute.defendant}</p>
              </div>
              <div className="col-span-2">
                <p className="text-[10px] uppercase tracking-widest text-outline mb-1 font-bold">Motif du litige</p>
                <p className="text-primary font-bold">{currentDispute.reason}</p>
              </div>
              <div className="col-span-2 bg-surface-container-low/50 p-6 rounded-xl border border-outline-variant/20 relative">
                ...
                <p className="text-sm leading-relaxed text-on-surface-variant italic font-headline text-lg pl-8 relative z-10">
                  "{currentDispute.quoting || 'Pas de description supplémentaire.'}"
                </p>
              </div>
              <div className="col-span-2">
          ...
          <section className="bg-surface-container-lowest rounded-xl p-6 shadow-sm border border-outline-variant/10">
            ...
            <div className="space-y-6">
              <div className="p-4 rounded-lg bg-surface-container-low/50 flex items-center gap-4">
                <div className="w-12 h-12 rounded-full bg-secondary flex items-center justify-center text-white font-bold shadow-sm">
                  {currentDispute.plaintiff.substring(0, 2).toUpperCase()}
                </div>
                <div>
                  <h4 className="font-bold text-sm text-on-surface">{currentDispute.plaintiff}</h4>
                  <p className="text-[10px] text-secondary font-semibold uppercase tracking-wider">{currentDispute.plaintiffRole || 'Acheteur'}</p>
              ...
              <div className="p-4 rounded-lg bg-surface-container-low/50 flex items-center gap-4">
                <div className="w-12 h-12 rounded-full bg-tertiary flex items-center justify-center text-white font-bold shadow-sm">
                  {currentDispute.defendant.substring(0, 2).toUpperCase()}
                </div>
                <div>
                  <h4 className="font-bold text-sm text-on-surface">{currentDispute.defendant}</h4>
                  <p className="text-[10px] text-tertiary font-semibold uppercase tracking-wider">{currentDispute.defendantRole || 'Transporteur'}</p>
                  <div className="flex items-center gap-1 mt-1">
                    <span className="material-symbols-outlined text-amber-500 text-xs" style={{ fontVariationSettings: "'FILL' 1" }}>star</span>
          ...
          <section className="bg-surface-container-lowest rounded-xl p-6 shadow-sm border-2 border-primary/20">
            ...
            <div className="space-y-4">
              <label className="group relative flex items-center gap-4 p-4 rounded-lg border border-outline-variant hover:border-primary transition-all cursor-pointer bg-white">
                <input type="radio" name="decision" value="refund" onChange={(e) => setDecision(e.target.value)} className="w-4 h-4 text-primary focus:ring-primary border-outline-variant accent-primary" />
              ...
              <label className="group relative flex items-center gap-4 p-4 rounded-lg border border-outline-variant hover:border-primary transition-all cursor-pointer bg-white">
                <input type="radio" name="decision" value="release" onChange={(e) => setDecision(e.target.value)} className="w-4 h-4 text-primary focus:ring-primary border-outline-variant accent-primary" />
                <div className="flex flex-col">
              ...
              <label className="group relative flex items-center gap-4 p-4 rounded-lg border border-outline-variant hover:border-primary transition-all cursor-pointer bg-white">
                <input type="radio" name="decision" value="partial" onChange={(e) => setDecision(e.target.value)} className="w-4 h-4 text-primary focus:ring-primary border-outline-variant accent-primary" />
                <div className="flex flex-col">
              ...
              <button 
                ...
                className="w-full bg-primary text-white font-bold py-4 rounded-lg shadow-lg shadow-primary/20 hover:bg-primary-container active:scale-[0.98] transition-all flex items-center justify-center gap-2 mt-4">
